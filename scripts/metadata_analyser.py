...
def get_time(c):
    return datetime(int(c[0:4]), int(c[4:6]), int(c[6:8]),
                    int(c[8:10]), int(c[10:12]), int(c[12:14])).strftime(time_format)
...
def process_file(name, file_info):
    ...
    if (info_was_changed or file_was_changed) and not skip_entry:
        print(datetime.now().strftime(time_format), get_time(info['modify']), get_time(info['create']),
              info['size'], info_was_changed, file_was_changed, sep=',', file=file_log[name], flush=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for name in filenames:
            ...
            print('Log time,Modify time,Create time,Size,Metadata changed,Binary changed',
                  file=file_log[name], flush=True)
    except Exception as err:
        print(err)
        print(f'{datetime.now().strftime(time_format)}\t|\t{err}', file=err_log, flush=True)
        quit()
